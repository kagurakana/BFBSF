...
try
   ...
   disp("*** The first run will launch the Matlab Parallel Pool ***");
   disp("*** may takes few time. ***")
   parfor ii = 1:2
   tsh = 1;
   end
end
...
disp(TIME/length(psnrArr_M));


function p = fitplot(X,attr)
len = length(X);
p = polyfit(1:len,X,2);
y1 = polyval(p,1:len);
plot(y1,attr)
...
for i = 2:m_len + 1
    ...
    %     time_res = toc;
    %     disp(strcat("run ",methods_name_list(i-1)," in ",string(time_res)," sec."))
    if exist('I_clear','var')  && class(I_clear)~="char" && class(I_clear)~="string"
